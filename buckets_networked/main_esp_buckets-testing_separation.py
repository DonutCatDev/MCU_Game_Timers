...
class Game_States:
    ...
    def __init__(self):
        ...
        self.red_time = 0
        self.blue_time = 0
        self.esp_message = b"empty"
        self.esp_msg_dec = None
    ...
    def esp_read(self, cap=False):
        if ESP:
            output = None
            msg = ESP.read()
            if msg is not None and msg != self.esp_message:
                print(msg.msg)
                self.esp_message = msg.msg
                msg_dec = self.esp_message.decode()
                if msg_dec == "Start":
                    output = "Start"
                elif msg_dec == "Pause":
                    ...
                    self.timer_state = True
                    if (cap and self.cap_state) or not cap:
                        RGBS.update(self.team, delay=0.0025)
                    else:
    ...
    async def w_start_wait(self):
        display_message("Waiting for timer...")
        RGBS.update(color1=self.team, pattern="single_blink_cycle", repeat=-1)
        while True:
            if self.esp_read() == "Start":
                await sleep(6)
                break
...
async def start_crazykingw(game_mode):
    ...
    local_state = initial_state.shallow_copy()
    await sleep(0.5)
    await local_state.w_start_wait()
    display_message(
        f"RED:  {local_state.red_time_str}\nBLUE: {local_state.blue_time_str}"
    )
    await sleep(0)
    ...
    while True:
        if local_state.timer_state:
            ...
            if monotonic() - clock >= 1:
                ...
                clock = monotonic()
        if local_state.pause_end_check():
            display_message("exiting...")
            await sleep(0.5)
            break
...
async def start_hoth(game_mode):
    """Function for HotH game mode"""
    local_state = initial_state.shallow_copy()
    await sleep(0.5)
    display_message(
        f"RED:  {local_state.red_time_str}\nBLUE: {local_state.blue_time_str}"
...
async def start_hothw(game_mode):
    """Function for HotH game mode, wireless version"""
    local_state = initial_state.shallow_copy()
    await sleep(0.5)
    await local_state.w_start_wait()
    display_message(
...
async def start_hotpocketsw(game_mode):
    """Function for HotPocket game mode, wireless version"""
    local_state = initial_state.shallow_copy()
    await local_state.w_start_wait()
    await sleep(0.5)
    display_message(f"Countdown\n{local_state.game_length_str}")
    ...
    clock = monotonic()
    await sleep(0)
    while local_state.game_length > 0:  # Pre-capture countdown
        if monotonic() - clock >= 1:
            local_state.game_length -= 1
            display_message(f"Countdown\n{local_state.game_length_str}")
            clock = monotonic()
    ...
    hold_time = 0
    await sleep(0)
    while True:  # Capture available with long press
        if REDB.rose or BLUEB.rose:  # See button release outside timer_state
            local_state.cap_state = False
        if local_state.timer_state:
            if REDB.fell or BLUEB.fell:
                hold_time = monotonic()
...
MODES = [
    ...
    GameMode(
        "HotPockets W", has_game_length=True, has_timerbox=True, has_long_press=True
    ),
]
# endregion
"""
